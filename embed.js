...
function embed() {
    ...
    const textMetaData = user + "/" + repository + " : " + file;
...
`);
    ...
    Promise.all(showLineNumbers ? [fetchFile, loadHLJS, loadHLJSNum] : [fetchFile, loadHLJS]).then((result) => {
        ...
        embedCodeToTarget(targetDiv, result[0], showBorder, showLineNumbers, showFileMeta, showCopy, isDarkStyle, target.href, rawFileURL, fileExtension, startLine, endLine, tabSize, height, textMetaData);
    }).catch((error) => {
        const errorMsg = `Failed to process ${rawFileURL}
${error}`;
        const targetDiv = document.getElementById(containerId);
        embedCodeToTarget(targetDiv, errorMsg, showBorder, showLineNumbers, showFileMeta, showCopy, isDarkStyle, target.href, rawFileURL, 'plaintext', -1, -1, tabSize, height, textMetaData);
    });
}
...
function embedCodeToTarget(targetDiv, codeText, showBorder, showLineNumbers, showFileMeta, showCopy, isDarkStyle, fileURL, rawFileURL, lang, startLine, endLine, tabSize, height, textMetaData) {
    targetDiv.innerHTML = "";
...
<a target="_blank" href="${fileURL}">${textMetaData}</a>`;
